...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def pre_annotate(self):
        models_conf = os.path.join(os.path.dirname(__file__), "data", "models.conf")
        if not os.path.exists(models_conf):
            QMessageBox.warning(self, u"警告", u"未找到 data/models.conf 预标注模型配置文件")
            return
        ...
        with codecs.open(models_conf, 'r', 'utf8') as f:
            for line in f:
                ...
                if '=' in line:
                    ...
                    if key.strip() == 'model':
                        model_rel_path = val.strip()
                        break

        if not model_rel_path:
            QMessageBox.warning(self, u"警告", u"models.conf 中未找到 model 配置")
            return
