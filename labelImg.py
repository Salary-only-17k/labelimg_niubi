...
class PreAnnotateDialog(QDialog):
    def __init__(self, labels, img_count, parent=None):
        super(PreAnnotateDialog, self).__init__(parent)
        self.setWindowTitle(u"预标注设置")
        self.setMinimumWidth(400)
        self.setMinimumHeight(350)

        layout = QVBoxLayout(self)

        lbl = QLabel(u"选择需要预标注的标签（可多选）：")
        layout.addWidget(lbl)

        self.select_all_cb = QCheckBox(u"全选")
        self.select_all_cb.stateChanged.connect(self.on_select_all_changed)
        layout.addWidget(self.select_all_cb)

        self.label_list = QListWidget()
        for lbl_name in labels:
            item = QListWidgetItem(lbl_name)
        ...
        self.start_spin.setRange(1, img_count)
        self.start_spin.setValue(1)
        form_layout.addRow(u"开始图像位置:", self.start_spin)

        self.end_spin = QSpinBox()
        self.end_spin.setRange(1, img_count)
        self.end_spin.setValue(img_count)
        form_layout.addRow(u"结束图像位置:", self.end_spin)
    ...
    def on_select_all_changed(self, state):
        checked = Qt.Checked if state == Qt.Checked else Qt.Unchecked
        for i in range(self.label_list.count()):
            self.label_list.item(i).setCheckState(checked)
    ...
    def start_idx(self):
        return self.start_spin.value() - 1

    def end_idx(self):
        return self.end_spin.value() - 1


class RangeDialog(QDialog):
    def __init__(self, label, img_count, parent=None):
        super(RangeDialog, self).__init__(parent)
        self.setWindowTitle(u"YOLOE 自动标注范围")
        self.setMinimumWidth(350)

        layout = QVBoxLayout(self)

        layout.addWidget(QLabel(u"目标标签: %s" % label))
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        self.adjust_dock = QDockWidget(u"图像调整", self)
        self.adjust_dock.setObjectName(u"adjust")
        adjust_widget = QWidget()
        adjust_layout = QVBoxLayout(adjust_widget)

        brightness_label = QLabel(u"亮度")
        ...
        dehaze_label = QLabel(u"去雾")
        adjust_layout.addWidget(dehaze_label)
        self.dehaze_slider = QSlider(Qt.Horizontal)
        self.dehaze_slider.setRange(0, 100)
        self.dehaze_slider.setValue(0)
        self.dehaze_slider.valueChanged.connect(self._on_adjustment_changed)
        adjust_layout.addWidget(self.dehaze_slider)

        self.grayscale_cb = QCheckBox(u"灰度化")
        self.grayscale_cb.stateChanged.connect(self._on_adjustment_changed)
        adjust_layout.addWidget(self.grayscale_cb)

        reset_btn = QPushButton(u"复位")
        reset_btn.clicked.connect(self._reset_adjustments)
        adjust_layout.addWidget(reset_btn)

        adjust_layout.addWidget(QLabel(u"辅助线"))
        self.guideline_combo = QComboBox()
        self.guideline_combo.addItem(u"无", 0)
        self.guideline_combo.addItem(u"上 1/3", 3)
        self.guideline_combo.addItem(u"上 1/4", 4)
        self.guideline_combo.addItem(u"上 1/5", 5)
        self.guideline_combo.currentIndexChanged.connect(self._on_guideline_changed)
        adjust_layout.addWidget(self.guideline_combo)

        adjust_layout.addStretch()
        self.adjust_dock.setWidget(adjust_widget)
        self.addDockWidget(Qt.RightDockWidgetArea, self.adjust_dock)
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, open_annotation, copy_prev_bounding, None, pre_annotate, self.menus.recentFiles, save, save_format, save_as, close, reset_all, delete_image, quit))
    ...
    def _reset_adjustments(self):
        ...
        self.guideline_combo.setCurrentIndex(0)

    def _on_guideline_changed(self):
        self.canvas.guideline_denom = self.guideline_combo.currentData()
        self.canvas.update()

    def _on_adjustment_changed(self):
        self._apply_image_adjustments()

    def _min_filter_1d(self, arr, k):
        pad = k // 2
    ...
    def _min_filter(self, img, k):
        rows = self._min_filter_1d(img, k)
        cols = self._min_filter_1d(rows.T, k).T
        return cols

    def _dark_channel(self, img, patch_size):
        return self._min_filter(img, patch_size)

    def _dehaze(self, bgr, strength):
        ...
        dark = self._dark_channel(min_ch, 7)
        ...
        A = np.mean(f[brightest], axis=0)

        norm = f / (A + 1e-6)
        norm_dark = self._dark_channel(norm.min(axis=2), 7)
        t = 1.0 - strength * norm_dark
        t = np.clip(t, 0.1, 1.0)

        result = np.zeros_like(f)
        for c in range(3):
            result[:,:,c] = (f[:,:,c] - A[c]) / t + A[c]

        result = np.clip(result * 255, 0, 255).astype(np.uint8)
        return result

    def _apply_image_adjustments(self):
        if self.orig_image.isNull():
            return
        ...
        if brightness == 100 and saturation == 100 and not grayscale and dehaze == 0:
        ...
        img = self.orig_image.convertToFormat(QImage.Format_ARGB32)
        ...
        if dehaze > 0:
            bgr = self._dehaze(bgr, dehaze)

        if brightness != 100:
            bgr = np.clip(bgr.astype(np.float32) * brightness / 100.0, 0, 255).astype(np.uint8)

        if saturation != 100:
            cf = bgr.astype(np.float32) / 255.0
            gray = cf.dot([0.114, 0.587, 0.299])[:,:,np.newaxis]
            bgr = np.clip(gray + (cf - gray) * saturation / 100.0, 0, 1) * 255
        ...
        arr[:,:,:3] = bgr
        result = QImage(arr.data.tobytes(), w, h, QImage.Format_ARGB32)
    ...
    def delete_image(self):
        if self.file_path is None:
            return
        reply = QMessageBox.question(self, u"确认删除",
    ...
    def pre_annotate(self):
        ...
        if not os.path.exists(models_conf):
            QMessageBox.warning(self, u"警告", u"未找到 data/pre_label_models.conf 预标注配置文件")
            return

        if self.img_count <= 0:
            QMessageBox.warning(self, u"警告", u"请先打开图像目录")
            return

        config = {}
        with codecs.open(models_conf, 'r', 'utf8') as f:
            for line in f:
                line = line.strip()
                if '=' in line:
        ...
        model_rel_path = config.get('model')
        if not model_rel_path:
            QMessageBox.warning(self, u"警告", u"pre_label_models.conf 中未找到 model 配置")
            return
        ...
        if not os.path.exists(model_path):
            QMessageBox.warning(self, u"警告", u"未找到模型文件: %s" % model_path)
            return

        _old_path = sys.path[:]
        sys.path = [p for p in sys.path if os.path.realpath(p) != os.path.realpath('.')]
        try:
            # sys.path.append(".")
            from ultralytics import YOLO
        except ImportError:
        ...
        if ',' in raw_imgsz:
            parts = [int(x.strip()) for x in raw_imgsz.split(',')]
            imgsz = (parts[0], parts[1]) if len(parts) >= 2 else parts[0]
        else:
            imgsz = int(raw_imgsz)

        self.statusBar().showMessage(u"正在加载模型...")
        model = YOLO(model_path)

        labels = list(model.names.values())

        dlg = PreAnnotateDialog(labels, self.img_count, self)
        if dlg.exec_() != QDialog.Accepted:
            return

        selected_labels = dlg.selected_labels()
        start_idx = dlg.start_idx()
        end_idx = dlg.end_idx()
        count = end_idx - start_idx + 1
        img_list = self.m_img_list[start_idx:end_idx + 1]

        if self.default_save_dir is None:
            if self.dir_name:
                self.default_save_dir = self.dir_name
            elif self.file_path:
                self.default_save_dir = os.path.dirname(self.file_path)

        current_file = self.file_path

        self.statusBar().showMessage(u"正在推理...")
        results = model(img_list, conf=conf, imgsz=imgsz, device=device)
        ...
        for i, result in enumerate(results):
            ...
            self.statusBar().showMessage(u"预标注进度: %d / %d" % (i + 1, count))
        ...
        QMessageBox.information(self, u"完成", u"预标注完成，共处理 %d 张图像" % count)

    def yoloe_auto_annotate(self):
        shape = self.canvas.selected_shape
        if shape is None:
            QMessageBox.warning(self, u"警告", u"请在画布上选择一个已标注的框作为YOLOE视觉提示")
            return

        label = shape.label
        pts = shape.points
        if len(pts) < 4:
            QMessageBox.warning(self, u"警告", u"选择的框需要4个顶点")
            return
        ...
        dlg = RangeDialog(label, self.img_count, self)
        if dlg.exec_() != QDialog.Accepted:
            return
        ...
        yoloe_conf = os.path.join(os.path.dirname(__file__), "data", "yoloe_models.conf")
        yoloe_config = {}
        if os.path.exists(yoloe_conf):
            with codecs.open(yoloe_conf, 'r', 'utf8') as f:
                for line in f:
                    line = line.strip()
                    if '=' in line:
                        k, v = line.split('=', 1)
                        yoloe_config[k.strip()] = v.strip()

        yoloe_model_rel = yoloe_config.get('model', 'yoloe-26l-seg.pt')
        yoloe_model_path = yoloe_model_rel
        if not os.path.isabs(yoloe_model_path):
            yoloe_model_path = os.path.join(os.path.dirname(__file__), yoloe_model_path)

        yoloe_conf_val = float(yoloe_config.get('conf', 0.5))
        yoloe_device = yoloe_config.get('device', 'cpu')
        raw_imgsz = yoloe_config.get('imgsz', '640')
        if ',' in raw_imgsz:
            parts = [int(x.strip()) for x in raw_imgsz.split(',')]
            yoloe_imgsz = (parts[0], parts[1]) if len(parts) >= 2 else parts[0]
        else:
            yoloe_imgsz = int(raw_imgsz)

        self.statusBar().showMessage(u"正在加载YOLOE模型...")
        QApplication.processEvents()
        ...
        current_file = self.file_path
        saved_format = self.label_file_format

        self.statusBar().showMessage(u"YOLOE 正在推理...")
        QApplication.processEvents()
        ...
        for i, result in enumerate(results):
            ...
            if boxes is not None and len(boxes) > 0:
                for j in range(len(boxes)):
                    if int(boxes.cls[j]) != 0:
                        continue
                    xyxy = boxes.xyxy[j].tolist()
                    bx1, by1, bx2, by2 = xyxy
                    s = Shape(label=label)
                    s.add_point(QPointF(bx1, by1))
                    s.add_point(QPointF(bx2, by1))
                    s.add_point(QPointF(bx2, by2))
                    s.add_point(QPointF(bx1, by2))
                    s.close()
                    s.line_color = generate_color_by_text(label)
                    s.fill_color = generate_color_by_text(label)
            ...
            self.statusBar().showMessage(u"YOLOE 自动标注进度: %d / %d" % (i + 1, count))
            QApplication.processEvents()

        self.label_file_format = saved_format
        ...
        QMessageBox.information(self, u"完成", u"YOLOE自动标注完成，共处理 %d 张图像" % count)
